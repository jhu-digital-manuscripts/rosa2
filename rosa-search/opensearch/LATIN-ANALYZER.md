...
Both configurations are verified against OpenSearch 3.x. The `stemmer_override` token
filter and the `mapping` character filter used here are unchanged from OpenSearch 2.x,
and produce identical tokens on both versions.

## Overview

Latin presents unique challenges for text analysis:
- Rich morphology with extensive noun declensions and verb conjugations
- No built-in stemmer support in OpenSearch
...
```yaml
volumes:
  - ../rosa-search/opensearch/latin-stemmer-rules.txt:/usr/share/opensearch/config/analyzers/latin-stemmer-rules.txt:ro,Z
  - ../rosa-search/opensearch/latin-charmap.txt:/usr/share/opensearch/config/analyzers/latin-charmap.txt:ro,Z
```

This is what `local-dev/docker-compose.yml` does. The `Z` suffix applies a private
SELinux label and is required on SELinux-enforcing hosts such as Fedora and RHEL; it is
harmless elsewhere. Because the label is private to one container, do not mount these
same files into a second container with `:Z` — doing so relabels them and the first
container loses read access, which makes index creation fail with
`IOException while reading mappings_path: file not readable`. Give additional containers
their own copies instead.
...
aws s3 cp rosa-search/opensearch/latin-stemmer-rules.txt s3://your-bucket/opensearch-packages/latin-stemmer-rules.txt
aws s3 cp rosa-search/opensearch/latin-charmap.txt s3://your-bucket/opensearch-packages/latin-charmap.txt
```
...
```yaml
volumes:
  - ../rosa-search/opensearch/latin-stopwords.txt:/usr/share/opensearch/config/analyzers/latin-stopwords.txt:ro,Z
  - ../rosa-search/opensearch/latin-charmap.txt:/usr/share/opensearch/config/analyzers/latin-charmap.txt:ro,Z
```
...
This reads `rosa-search/latin-lemma-table.jsonl` and outputs to `rosa-search/opensearch/latin-stemmer-rules.txt`.
