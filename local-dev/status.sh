...
if docker compose -f "$SCRIPT_DIR/docker-compose.yml" ps -q 2>/dev/null | grep -q .; then
    ...
    if curl -s "http://localhost:$OPENSEARCH_PORT/_cluster/health" > /dev/null 2>&1; then
        version=$(curl -s "http://localhost:$OPENSEARCH_PORT/" | grep -o '"number"[[:space:]]*:[[:space:]]*"[^"]*"' | cut -d'"' -f4)
        lucene=$(curl -s "http://localhost:$OPENSEARCH_PORT/" | grep -o '"lucene_version"[[:space:]]*:[[:space:]]*"[^"]*"' | cut -d'"' -f4)
        if [[ -n "$version" ]]; then
            status_ok "Version: Opensearch $version (Lucene $lucene)"
        fi
        ...
        case "$health" in
            green)
                ...
                ;;
            yellow)
                # Expected on a single node: the index definitions do not override
                # number_of_replicas, so the default replica stays unassigned.
                status_ok "Cluster health: yellow (expected for a single-node cluster)"
                ;;
            red)
