...
# and open http://localhost:3001 to choose the AoR or DLMM site
#
# Usage:
...
SEARCH_DIR="$REPO_ROOT/rosa-search"
...
# Create a single Opensearch index from a definition file.
#
# Failing loudly here matters: if index creation fails, the subsequent bulk
# ingest silently auto-creates the index with dynamic mappings, which breaks
# faceting and sorting in ways that are not obvious until you use the viewer.
create_index() {
    local name="$1"
    local definition="$2"
    local response

    response=$(curl -s -w '\n%{http_code}' -X PUT "http://localhost:$OPENSEARCH_PORT/$name" \
    ...
    if [[ "$status" != "200" ]]; then
        error "Failed to create index '$name' (HTTP $status):
...
create_indexes() {
    ...
    local index_dir="$SEARCH_DIR/opensearch"
    ...
    create_index manifest "$index_dir/manifest.json"
    create_index canvas "$index_dir/canvas.json"
...
ingest_data() {
    ...
    for f in "$OS_DIR"/*.bulk.json; do
        ...
        for chunk in "$chunk_dir"/chunk_*; do
            ...
            response=$(curl -s -w '\n%{http_code}' -X POST "http://localhost:$OPENSEARCH_PORT/_bulk" \
            ...
            if [[ "$status" != "200" ]]; then
                error "Bulk request failed for $filename (HTTP $status):

$body"
            fi
            
            # The Bulk API returns 200 even when individual documents fail,
            # so the response body has to be inspected.
            if [[ "$body" == *'"errors":true'* ]]; then
                local chunk_errors
                chunk_errors=$(grep -o '"error"' <<< "$body" | wc -l)
                failed_items=$((failed_items + chunk_errors))
                if [[ -z "$first_error" ]]; then
                    first_error=$(grep -o '"reason":"[^"]*"' <<< "$body" | head -1)
                fi
            fi
    ...
    if [[ $failed_items -gt 0 ]]; then
        log_warn "$failed_items document(s) failed to index. First error: $first_error"
    fi
...
main() {
    ...
    echo "    npm run dev"
    echo ""
    echo "  Then open http://localhost:3001 and choose a site:"
    echo "    AoR:   http://localhost:3001/aor/"
    echo "    DLMM:  http://localhost:3001/dlmm/"
    echo ""
