...
export function CanvasResult({ result, onClick }) {
  ...
  const { label, manifest_id, page_num, iiif_image_id } = source;
  ...
  // Build thumbnail URL
  const thumbnailUrl = iiif_image_id ? buildThumbnailUrl(iiif_image_id, thumbnailConfig) : null;
    ...
    <Card variant="outlined" sx={{ mb: 1 }}>
      <CardActionArea onClick={() => onClick(result)} sx={{ p: 1.5 }}>
        <Box sx={{ display: 'flex', gap: 2 }}>
          {/* Thumbnail */}
          {thumbnailUrl && <LazyThumbnail src={thumbnailUrl} alt={`Page ${label}`} />}
          ...
          <Box sx={{ flex: 1, minWidth: 0 }}>
            ...
            {highlightEntries.map(([field, fragments]) => (
              <HighlightedText key={field} html={fragments[0]} fieldLabel={field.split('.')[0]} />
            ))}
          </Box>
        </Box>
      </CardActionArea>
