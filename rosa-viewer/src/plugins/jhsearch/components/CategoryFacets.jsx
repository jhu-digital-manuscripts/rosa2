...
import { formatFacetValue, getQuantizeInterval } from '../utils/facets';
...
function FacetCategory({ category, values, selectedValues, onToggle }) {
  ...
  const isQuantized = getQuantizeInterval(category) !== null;

  // Quantized categories are numeric ranges, so they only make sense in numeric order.
  // Everything else is ordered by count (descending), then alphabetically.
      ...
      <AccordionDetails sx={{ p: 0 }}>
        <List dense disablePadding>
          {sortedValues.map((value) => {
            ...
            // Convert key to string for React key
            const keyStr = String(value.key);
            const displayValue = formatFacetValue(value.key, category);
          ...
          {sortedValues.length === 0 && (
            <ListItem>
              <ListItemText secondary="No values available" sx={{ textAlign: 'center' }} />
            </ListItem>
          )}
        </List>
      </AccordionDetails>
  ...
  for (const [categoryName, values] of Object.entries(facets)) {
    ...
    for (const value of values) {
      chips.push({
        ...
        categoryLabel,
        value,
        displayValue: formatFacetValue(value, category),
      });
    }
  }
    ...
    <Box sx={{ mb: 2 }}>
      ...
      <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
        {chips.map((chip) => (
          <Chip
            key={chip.key}
            label={`${chip.categoryLabel}: ${chip.displayValue}`}
            size="small"
...
export function CategoryFacets() {
  ...
  return (
    <Box>
      ...
      <SelectedFacets facets={facets} categories={categories} onToggle={handleToggle} />
