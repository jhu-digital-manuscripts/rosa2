...
rosa2 provides full-text and faceted search over manuscript collection data through an Opensearch 3.x backend. The search system combines three components:
...
- **Profile**: `https://github.com/jhu-digital-manuscripts/rosa2/docs/search.md`
...
The system uses two Opensearch indexes, each defined in the `rosa-search/opensearch/` directory:

| Index | Definition File | Description |
|-------|----------------|-------------|
| `manifest` | `rosa-search/opensearch/manifest.json` | One document per book — bibliographic metadata, identifiers, and keyword facet fields |
...
See `docs/opensearch-indexes.md` for complete field definitions.
...
```bash
java -jar rosa-tool.jar generate-iiif-pres \
...
```bash
java -jar rosa-tool.jar generate-opensearch-ingest \
  --archive /data/archive \
  --output /deploy/bulk
```
...
Verify that both requests return HTTP 200. See
[docs/opensearch-indexes.md](opensearch-indexes.md#creating-indexes) for why a silent
failure here is damaging.
...
Treat `_score` as a relative ranking signal only. Do not display it, persist it, or
compare it against a fixed threshold — its absolute scale is an implementation detail
of the underlying Lucene version. Two examples from the Opensearch 2 to 3 upgrade:

- Lucene 10 removed the `(k1 + 1)` factor from the BM25 numerator, so text-field scores
  are uniformly about 2.2× smaller than on Opensearch 2. Result ordering is unchanged.
- Term queries on `keyword` fields now rewrite to a constant score of `1.0` instead of
  an idf-weighted BM25 score. This shifts how a `field.keyword` match ranks relative to
  a text sub-field match within the same `field.*` `multi_match`.

Also note that sorting by `_score` alone has no tiebreaker. Queries that match many
equally scoring documents — for example a `mark.keyword` lookup where every hit scores
the same — return an arbitrary page of those hits, and the arbitrary order is not stable
across versions or shard layouts. Add a secondary sort key such as `id` if stable
