...
```
java -jar rosa-tool.jar generate-iiif-pres --archive <dir> --output <dir> [--base-url <url>] [--image-base-url <url>] [--image-api-version <2|3>] [--opensearch-url <url>]
```
...
| `--image-base-url` | No | Base URL for IIIF Image API services. Defaults to `--base-url`. |
| `--image-api-version` | No | IIIF Image API version: `2` (default) or `3` |
| `--opensearch-url` | No | Opensearch `_search` endpoint URL. When given, a `service/jhsearch.json` file is written per collection and referenced from collections and manifests (see [Search](search.md)). |
...
```
java -jar rosa-tool.jar generate-opensearch-ingest --archive <dir> --output <dir>
```
...
```
java -jar rosa-tool.jar shallow-copy --archive <dir> --output <dir>
```
...
```
java -jar rosa-tool.jar check --archive <dir> [--check-bits]
```
...
```
java -jar rosa-tool.jar list --archive <dir> [--collection <id>]
```
...
```
java -jar rosa-tool.jar validate-xml --archive <dir> --collection <id>
```
...
```
java -jar rosa-tool.jar aor-stats <book-dir> [<book-dir> ...]
```
...
```
java -jar rosa-tool.jar update --archive <dir> [--collection <id>] [--book <id>] [--force]
```
...
```
java -jar rosa-tool.jar update-image-list --archive <dir> --collection <id> --book <id> [--force]
```
...
```
java -jar rosa-tool.jar crop-images --archive <dir> --collection <id> [--book <id>] [--force]
```
...
```
java -jar rosa-tool.jar file-map --archive <dir> --collection <id> --book <id>
```
...
```
java -jar rosa-tool.jar rename-images --archive <dir> --collection <id> --book <id> [--change-id] [--reverse]
```
...
```
java -jar rosa-tool.jar rename-files <directory> <csv-file>
```
...
```
java -jar rosa-tool.jar rename-transcriptions --archive <dir> --collection <id> --book <id> [--reverse]
```
...
```
java -jar rosa-tool.jar generate-tei --archive <dir> --collection <id> --book <id>
```
...
```
java -jar rosa-tool.jar check-aor --archive <dir> --collection <id> [--book <id>] [--spreadsheet-dir <dir>]
```
...
```
java -jar rosa-tool.jar generate-annotation-map --archive <dir> --collection <id>
```
...
```
java -jar rosa-tool.jar migrate-tei-metadata --archive <dir> --collection <id> [--book <id>]
```
...
```
java -jar rosa-tool.jar decorate-image-list --archive <dir> --collection <id> [--book <id>]
```
