    ...
    public void write(ImageList imageList, OutputStream out) throws IOException {
        ...
        for (BookImage image : imageList) {
            ...
            if (image.getName() != null && !image.getName().isEmpty()) {
                if (image.isAutoGeneratedName()) {
                    label += GENERATED_PREFIX;
                }
                label += image.getName();
            }

            line += image.getId() + DELIMITER +
                    image.getWidth() + DELIMITER +
                    image.getHeight() + DELIMITER +
                    label +
    ...
    private BookImage buildBookImage(String[] csvRow, List<String> errors) {
        ...
        if (csvRow.length > 3) {
            ...
            if (label.startsWith(GENERATED_PREFIX)) {
                label = label.substring(GENERATED_PREFIX.length());
