...
public class ImageListDecoratorTest extends BaseArchiveTest {
    ...
    public void itDoesWhatItsSupposedToTest() throws Exception {
        ...
        assertFalse("The image named '322' should NOT be marked as having an autogenerated name",
                after.getImages().stream()
                        .filter(image -> image.getName().equals("322"))
                        .map(BookImage::isAutoGeneratedName)
                        .findFirst()
                        .orElse(true) // This fallback will fail the assertion
        );

        assertTrue(after.getImages().stream().anyMatch(image -> image.getName().equals("front matter 1v")));
        assertTrue("The image named 'front matter 1v' should be marked as having an autogenerated name",
                after.getImages().stream()
                        .filter(image -> image.getName().equals("front matter 1v"))
                        .map(BookImage::isAutoGeneratedName)
                        .findFirst()
                        .orElse(false)
        );
